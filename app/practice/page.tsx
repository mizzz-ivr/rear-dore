...
type PracticeRecordUpdateKind = "first" | "score-best" | "rare-best" | "saved" | null;

const practiceRecordUpdateMessage: Record<Exclude<PracticeRecordUpdateKind, null>, string> = {
  first: "初回の練習記録を保存しました",
  "score-best": "自己ベスト更新！",
  "rare-best": "SR以上の最高記録を更新！",
  saved: "今回の練習を記録しました",
};

export default function PracticePage() {
  const [unlockedQuestionSets, setUnlockedQuestionSets] = useState<QuestionSet[]>([]);
  const [practiceRecords, setPracticeRecords] = useState<PracticeThemeRecord[]>([]);
  const [selectedQuestionSet, setSelectedQuestionSet] = useState<QuestionSet | null>(null);
  const [questionIndex, setQuestionIndex] = useState(0);
  const [selectedChoiceId, setSelectedChoiceId] = useState<string | null>(null);
  const [answers, setAnswers] = useState<AnswerResult[]>([]);
  const [completed, setCompleted] = useState(false);
  const [completedRecord, setCompletedRecord] = useState<PracticeThemeRecord | null>(null);
  const [recordUpdateKind, setRecordUpdateKind] = useState<PracticeRecordUpdateKind>(null);
  const [historyReady, setHistoryReady] = useState(false);
  const [historyReadFailed, setHistoryReadFailed] = useState(false);
  const [recordsStorageFailed, setRecordsStorageFailed] = useState(false);
  ...
  useEffect(() => {
    ...
    let nextPracticeRecords: PracticeThemeRecord[] = [];
    let nextHistoryReadFailed = false;
    let nextRecordsStorageFailed = false;
    ...
    try {
      nextPracticeRecords = restorePracticeRecords(
        window.localStorage.getItem(PRACTICE_RECORDS_STORAGE_KEY),
        questionSets,
      );
    } catch {
      nextRecordsStorageFailed = true;
    }
    ...
    const frameId = window.requestAnimationFrame(() => {
      ...
      setRecordsStorageFailed(nextRecordsStorageFailed);
  ...
  useEffect(() => {
    function handleStorage(event: StorageEvent): void {
      if (event.key !== PRACTICE_RECORDS_STORAGE_KEY && event.key !== null) return;

      setPracticeRecords(restorePracticeRecords(event.newValue, questionSets));
      setRecordsStorageFailed(false);
    }
  ...
  function savePracticeRecord(): void {
    if (!selectedQuestionSet) return;

    try {
      const restored = restorePracticeRecords(
        window.localStorage.getItem(PRACTICE_RECORDS_STORAGE_KEY),
        questionSets,
      );
      ...
      window.localStorage.setItem(
        PRACTICE_RECORDS_STORAGE_KEY,
        serializePracticeRecords(update.records, questionSets),
      );

      const updateKind: Exclude<PracticeRecordUpdateKind, null> =
        update.previousRecord === null
          ? "first"
          : totalScore > update.previousRecord.bestScore
            ? "score-best"
            : rareAnswerCount > update.previousRecord.bestRareAnswerCount
              ? "rare-best"
              : "saved";

      setPracticeRecords([...update.records]);
      setCompletedRecord(update.record);
      setRecordUpdateKind(updateKind);
      setRecordsStorageFailed(false);
    } catch {
      setCompletedRecord(null);
      setRecordUpdateKind(null);
      setRecordsStorageFailed(true);
    }
  }
      ...
      <main id="main-content" className="min-h-screen px-4 py-8 sm:px-6 sm:py-14" tabIndex={-1}>
        <section className="mx-auto flex w-full max-w-2xl flex-col gap-6">
          <header>
            <p className="text-xs font-medium tracking-[0.22em] text-sky-300">PRACTICE MODE</p>
            <h1
              ref={themesHeadingRef}
              tabIndex={-1}
              className="focus-target mt-1 text-3xl font-black tracking-tight sm:text-4xl"
            >
              練習モード
            </h1>
            <p className="mt-2 text-sm leading-6 text-zinc-400">
              一度クリアしたテーマを、公式記録に影響させず何度でも遊べます。
            </p>
          </header>

          <div className="rounded-2xl border border-sky-300/20 bg-sky-300/8 p-4 text-sm leading-6 text-zinc-300">
            <p className="font-semibold text-sky-200">テーマ別の練習記録だけ保存します</p>
            <p className="mt-1">
              自己ベスト、練習回数、SR以上の最高回答数だけをこのブラウザに保存します。公式デイリーの履歴、連続日数、実績、今日の回答進捗には反映しません。
            </p>
          </div>
          ...
          {recordsStorageFailed && (
            <div
              className="rounded-2xl border border-amber-300/25 bg-amber-300/8 p-4 text-sm leading-6 text-amber-100"
              role="status"
            >
              このブラウザの保存領域を利用できないため、練習記録を保存・復元できません。練習自体はそのまま続けられます。
            </div>
          )}

          <section
            aria-labelledby="unlocked-themes-heading"
            className="rounded-[2rem] border border-white/10 bg-zinc-950/75 p-5 sm:p-8"
          >
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-xs font-medium tracking-[0.18em] text-zinc-500">UNLOCKED</p>
                <h2 id="unlocked-themes-heading" className="mt-1 text-xl font-bold">
                  プレイ済みテーマ
                </h2>
              </div>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {unlockedQuestionSets.map((questionSet) => {
                  const record = practiceRecords.find(
                    (candidate) => candidate.questionSetId === questionSet.id,
                  );
                  ...
                  return (
                    ...
                    >
                      <span className="block text-xs font-medium tracking-[0.16em] text-sky-300">
                        {record ? `BEST ${record.bestScore.toLocaleString("ja-JP")}点` : "5 QUESTIONS"}
                      </span>
                      <span className="mt-2 block text-lg font-bold">{questionSet.title}</span>
                      {record ? (
                        <span className="mt-2 block text-sm leading-6 text-zinc-400">
                          練習 {record.playCount.toLocaleString("ja-JP")}回 · SR以上最高 {record.bestRareAnswerCount}/5
                        </span>
                      ) : (
                        <span className="mt-1 block text-sm text-zinc-400">練習記録なし</span>
                      )}
        ...
        <section className="mx-auto flex w-full max-w-2xl flex-col gap-6">
          ...
          {recordsStorageFailed && (
            <div
              ...
              role="status"
            >
              今回の練習記録を保存できませんでした。結果の確認や再挑戦はそのまま続けられます。
            </div>
          )}
          ...
          {completedRecord && recordUpdateKind && (
            <section
              className="rounded-[2rem] border border-lime-300/20 bg-lime-300/8 p-5 sm:p-7"
              role="status"
              aria-live="polite"
              aria-labelledby="practice-record-title"
            >
              <p className="text-xs font-medium tracking-[0.18em] text-lime-300">THEME RECORD</p>
              <h2 id="practice-record-title" className="mt-1 text-xl font-bold text-lime-100">
                {practiceRecordUpdateMessage[recordUpdateKind]}
              </h2>
              <dl className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
                <div className="rounded-2xl bg-black/20 p-4">
                  <dt className="text-sm text-zinc-400">自己ベスト</dt>
                  <dd className="mt-1 text-xl font-bold tabular-nums">
                    {completedRecord.bestScore.toLocaleString("ja-JP")}点
                  </dd>
                </div>
                <div className="rounded-2xl bg-black/20 p-4">
                  <dt className="text-sm text-zinc-400">練習回数</dt>
                  <dd className="mt-1 text-xl font-bold tabular-nums">
                    {completedRecord.playCount.toLocaleString("ja-JP")}回
                  </dd>
                </div>
                <div className="rounded-2xl bg-black/20 p-4">
                  <dt className="text-sm text-zinc-400">SR以上最高</dt>
                  <dd className="mt-1 text-xl font-bold tabular-nums">
                    {completedRecord.bestRareAnswerCount}/5
                  </dd>
                </div>
              </dl>
            </section>
      ...
      <section className="mx-auto flex w-full max-w-2xl flex-col gap-6">
        <header className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-zinc-500">公式デイリーの記録には反映されません</p>
          </div>
          ...
          {!currentAnswer ? (
            ...
            <section className="mt-7 border-t border-white/10 pt-7">
              <p className="sr-only" role="status" aria-live="polite" aria-atomic="true">
                ...
                {currentAnswer.score.toLocaleString("ja-JP")}点獲得。
              </p>
              ...
              <div className="flex flex-col gap-3 rounded-2xl border border-sky-300/25 bg-sky-300/8 p-5 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div className="sm:text-right">
                  <p className="text-4xl font-black tabular-nums text-sky-300">
                    {formatPercentage(currentAnswer.percentage)}
                  </p>
