...
5問を完了すると、日付、問題セット、合計点、称号、レア度の並びを同一ブラウザのプレイ履歴へ保存します。同じ日付の再プレイは当日分を上書きし、日付降順で最大30件を保持します。総合結果画面には現在の連続プレイ日数、最長連続プレイ日数、自己ベスト、四捨五入した平均点と直近7件を表示します。個人統計は保存時と同じルールで日付を正規化し、同日データを二重集計しません。
